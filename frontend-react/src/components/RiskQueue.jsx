...
export default function RiskQueue({ issues }) {
  ...
  const totalRisk = issues.reduce((sum, issue) => sum + (issue.risk_score || 0), 0)
          ...
          issues.slice(0, 10).map((issue, idx) => (
            ...
            >
              <div className="issue-header">
                <span className="type-badge" style={{ background: getFailureColor(issue.failure_type) }}>
                  {issue.failure_type.toUpperCase()}
                </span>
                <span className="risk-score">₹{issue.risk_score.toFixed(0)}</span>
              </div>
